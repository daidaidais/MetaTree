    ...
    public void GenerateTree()
    {
        ...
        public void Update()
        {
            ...
            */
            ...
            /*
            1: volume
            2: speaking rate (rate_a)
            3: posture
            4: pulse rate
            */

            string[] parameters = UDPReceive.text.Split(',');
            if (int.Parse(parameters[0]) == 1)
            {
                BranchProbability = float.Parse(parameters[1]) * (float)0.1;
                Debug.Log("Average Volume is " + parameters[1] + ", BranchProbability is " + BranchProbability);
                //BaseRadius = float.Parse(parameters[1]);
            }
            else if (int.Parse(parameters[0]) == 2)
            {
                SegmentLength = (Mathf.Abs(Mathf.Abs(float.Parse(parameters[1]) - (float)0.5) - (float)0.5) / (float)0.5) * 2;
                Debug.Log("Talk Rate A is " + parameters[1] + ", SegmentLength is " + SegmentLength);
            }
            else if (int.Parse(parameters[0]) == 3)
            {
                Twisting = float.Parse(parameters[1]) * 40;
                //BranchProbability = float.Parse(parameters[1]) * (float)0.25;
                Debug.Log("Lean Y is " + parameters[1] + ", Twisting is " + Twisting);
            }
            BaseRadius = (float)(GetPulseSensor.BPM - 20) / (float)25;
            Debug.Log("BPM is " + GetPulseSensor.BPM + ", BaseRadius is " + BaseRadius);
